...
// precomputed linear feedback shift register wavetables
// each 1 bit generated by the LFSR toggles the output value
// poly5 is twice the length (62 entries) because its LFSR generates an odd number of 1 bits
static char poly4[(1 << 4) - 1];
static char poly5[((1 << 5) - 1) * 2];
...
float OscillatorPoly(OscillatorConfig const &config, OscillatorState &state, char poly[], int cycle, float step)
{
	if (step > 0.5f * cycle)
		return 0;
	float value = poly[state.loops] ? 1.0f : -1.0f;
#if ANTIALIAS == ANTIALIAS_POLYBLEP
	if (use_antialias)
	{
		float w = Min(step * POLYBLEP_WIDTH, 8.0f);

		// look backward for edges within range
		{
			float t = state.phase;
			int i = state.loops;
			char v0 = poly[i];
			while (t < w)
			{
				if (--i < 0)
					i += cycle;
				char const v1 = poly[i];
				if (v0 != v1)
					value += (v0 - v1) * PolyBLEP(t, w);
				v0 = v1;
				t += 1.0f;
			}
		}

		// look forward for edges within range
		{
			float t = state.phase - 1;
			int i = state.loops;
			char v0 = poly[i];
			while (t > -w)
			{
...
float OscillatorState::Update(OscillatorConfig const &config, float const frequency, float const step)
{
	...
	if (phase >= 1.0f)
	{
		...
		if (loops >= wave_loop_cycle[config.wavetype])
			loops -= wave_loop_cycle[config.wavetype];
	}
	else if (phase < 0.0f)
	{
		int advance = int(phase) + 1;
		phase -= advance;
		loops += advance;
		if (loops < 0)
			loops += wave_loop_cycle[config.wavetype];
